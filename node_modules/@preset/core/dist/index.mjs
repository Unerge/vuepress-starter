...
import fs$1, { createWriteStream, createReadStream } from 'node:fs';
...
const npmRunPath = ({
	...
	const cwdPath = path$3.resolve(cwdString);
	const result = [];
	...
	return [...result, pathOption].join(path$3.delimiter);
};
...
const applyExecPath = (result, execPath, cwdPath) => {
	...
	result.push(path$3.resolve(cwdPath, execPathString, '..'));
};
...
async function detectNodePackageManager(directory) {
  ...
  for (const [packageLock, packageManager2] of Object.entries(packageLockFiles)) {
    if (fs$1.existsSync(path$3.resolve(directory, packageLock))) {
  ...
  return {
    ...
    apply: async (context) => {
      ...
      try {
        ...
        if (!process.env.VITEST && !await fs$2.pathExists(context.applyOptions.targetDirectory)) {
          debug.preset(preset.name, "Target directory does not exist, creating it.");
          await fs$2.ensureDir(context.applyOptions.targetDirectory);
        }
...
async function importPresetFile(filepath) {
  ...
  const script = fs$1.readFileSync(filepath, { encoding: "utf-8" });
  const sanitizedScript = removeSelfImportStatement(script);
  return await evaluateConfiguration(sanitizedScript, path$3.dirname(filepath), filepath);
}
...
async function resolvePreset(options) {
  ...
  if (resolved.type === "file") {
    return emitAndReturn({
      rootDirectory: path$3.dirname(resolved.path),
...
async function resolveLocalDirectory(options, cwd) {
  const absolute = path$3.resolve(cwd, options.resolvable, options.parsedOptions.path ?? "");
  debug.resolve("Trying to resolve as a local directory:", absolute);
  try {
    if (fs$1.statSync(absolute).isDirectory()) {
...
async function resolveLocalFile(options, cwd) {
  ...
  try {
    if (fs$1.statSync(absolute).isFile()) {
...
async function resolvePresetFile(directory, cwd = process.cwd()) {
  ...
  const pkg = invoke(() => {
    try {
      return JSON.parse(fs$1.readFileSync(path$3.resolve(cwd, directory, "package.json")));
    } catch (error) {
    }
    return false;
  });
  const ensureFile = async (file) => {
    ...
    try {
      const filepath2 = path$3.resolve(cwd, directory, file);
      if (fs$1.statSync(filepath2).isFile()) {
        return filepath2;
      }
    } catch {
    }
...
async function cloneRepository(preset, options) {
  const targetDirectory = path$3.resolve(fs$1.realpathSync(os.tmpdir()), "presets", preset.repository);
  ...
  if (fs$1.statSync(targetDirectory, { throwIfNoEntry: false })?.isDirectory()) {
    debug.resolve(`${repositoryUrl} already exists (${targetDirectory}), deleting it.`);
    await fs$1.promises.rm(targetDirectory, { recursive: true, force: true });
  }
  ...
  try {
    await simpleGit().clone(repositoryUrl, targetDirectory, {
      "--depth": 1,
      ...tag && { "--branch": tag }
    });
  } catch (parent) {
  ...
  return path$3.resolve(targetDirectory, preset.path ?? "");
}
...
function renameDotfiles(input) {
  return input.replace(/^(.+[\/\\])*(.+\.dotfile)$/, (_, directory, dotfile) => path$3.resolve(`${directory}/.${dotfile.replace(".dotfile", "")}`));
}
const isFile$1 = (input) => fs$1.statSync(input, { throwIfNoEntry: false })?.isFile() ?? false;
const isDirectory = (input) => fs$1.statSync(input, { throwIfNoEntry: false })?.isDirectory() ?? false;
const extractTemplates = defineAction(
  "extract-templates",
  async ({ options, presetContext, actionContext }) => {
    const templatesPath = path$3.resolve(presetContext.localPreset.rootDirectory, options.templates, options.from);
    const targetPath = path$3.resolve(presetContext.applyOptions.targetDirectory, options.to);
    ...
    async function copyDirectoryToDirectory(input, output, glob) {
      ...
      for (const relativePath of paths) {
        const resolvedInput = path$3.resolve(input, relativePath);
        const resolvedOutput = path$3.resolve(output, relativePath);
    ...
    if (isFile$1(templatesPath) && isDirectory(targetPath)) {
      const finalTargetPath = options.flatten ? path$3.resolve(targetPath, path$3.basename(options.from)) : path$3.resolve(targetPath, options.from);
    ...
    await copyDirectoryToDirectory(
      path$3.resolve(presetContext.localPreset.rootDirectory, options.templates),
      targetPath,
      options.from
    );
    return true;
...
const editFiles = defineAction("edit-files", async ({ options, presetContext, actionContext }) => {
  ...
  debug.action(actionContext.name, `Matched ${paths.length} paths:`, paths);
  for (const relativePath of paths) {
    const targetFile = path$3.resolve(presetContext.applyOptions.targetDirectory, relativePath);
    let content = fs$1.readFileSync(targetFile, { encoding: "utf-8" });
    for (const operation of wrap(options.operations)) {
      ...
      if (operation.type === "edit-json") {
        ...
        if (operation.delete) {
          ...
          pathsToDelete.forEach((path2) => unset(json, path2));
        }
    ...
    fs$1.writeFileSync(targetFile, content, { encoding: "utf-8" });
  }
  return true;
});
...
const deletePaths = defineAction("delete-paths", async ({ options, actionContext, presetContext }) => {
  ...
  const deletePath = async (path2) => {
    ...
    return await fs$2.rm(path2, { force: true, recursive: true, maxRetries: 5 });
  };
  for (const pathOrGlobToDelete of pathsOrGlobs) {
    const absolutePathToDelete = path$3.resolve(presetContext.applyOptions.targetDirectory, pathOrGlobToDelete);
    if (await fs$2.pathExists(absolutePathToDelete)) {
    ...
    for (const pathToDelete of paths) {
      const absolutePathToDelete2 = path$3.resolve(presetContext.applyOptions.targetDirectory, pathToDelete);
...
const isFile = (input) => fs$1.statSync(input, { throwIfNoEntry: false })?.isFile() ?? false;
const renamePaths = defineAction("rename-paths", async ({ options, actionContext, presetContext }) => {
  ...
  const renamePath = async (oldPath, newPath) => {
    ...
    return await fs$2.rename(oldPath, newPath);
  };
  const getParsedPath = (pathOrGlob, absolutePathToRename) => {
    const ext = path$3.extname(pathOrGlob);

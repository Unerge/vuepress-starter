...
import { useRoute as useRoute4, useRouteLocale as useRouteLocale4 } from "vuepress/client";
var LICENSE_URL = {
  "CC0": {
...
function useCopyright(copyright) {
  ...
  const hasCopyright = computed14(() => Boolean(copyright.value));
  const creation = computed14(() => copyright.value.creation || "original");
  const license = computed14(() => resolveLicense(copyright.value.license, routeLocale.value));
  const author = computed14(() => resolveAuthor(copyright.value.author, creation.value, contributors.value));
  const sourceUrl = computed14(() => {
    if (creation.value === "original")
      return __VUEPRESS_SSR__ ? route.fullPath : location.href.split("#")[0];
    return copyright.value.source;
  });
  const creationText = computed14(() => {
...
import { computed as computed16, onMounted as onMounted2, onUnmounted as onUnmounted3 } from "vue";
var lightTint = {
...
import { resolveRoute, useRouteLocale as useRouteLocale5, withBase } from "vuepress/client";
...
function useLangs({
  ...
  const theme = useThemeData();
  const { page } = useData();
  const routeLocale = useRouteLocale5();
  const { isBlogPost } = useBlogPageData();
  const currentLang = computed18(() => {
...
import { computed as computed19, onMounted as onMounted3, ref as ref10, watchEffect as watchEffect2 } from "vue";
import { usePageLang as usePageLang3 } from "vuepress/client";
function useLastUpdated() {
  const { theme, page, frontmatter } = useData();
  const themeData2 = useThemeData();
  const lang = usePageLang3();
  const date = computed19(() => page.value.git?.updatedTime ? new Date(page.value.git.updatedTime) : null);
  const isoDatetime = computed19(() => date.value?.toISOString());
  const datetime = ref10("");
  const lastUpdatedText = computed19(() => {
...
import { computed as computed20, toValue as toValue2 } from "vue";
import { resolveRouteFullPath as resolveRouteFullPath2, useRoute as useRoute5 } from "vuepress/client";
var SEARCH_RE = /\.md(?:(?:#|\?).*)?$/;
function useLink(href, target) {
  const route = useRoute5();
  const { page } = useData();
  const isExternal = computed20(
    () => {
...
import { computed as computed21, ref as ref11, watch as watch5 } from "vue";
import { useRoute as useRoute6 } from "vuepress/client";
import { normalizeLink as normalizeLink3, resolveNavLink as resolveNavLink2 } from "../utils/index.js";
function useNavbarData() {
  const { theme } = useData();
  return computed21(() => resolveNavbar(theme.value.navbar || []));
}
...
import { resolveRouteFullPath as resolveRouteFullPath3, usePageLang as usePageLang4, useRoute as useRoute7 } from "vuepress/client";
...
function setupWatermark() {
  ...
  defineWatermarkConfig(computed23(() => {

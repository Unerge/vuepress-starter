...
var dePresetLocale = {
  ...
  "archive": "Archiv",
  "category": "Kategorie",
  "archiveTotal": "{count} Beitr\xE4ge",
  // ------ copyright license ------
...
var deDocsearchLocale = {
  ...
  translations: {
    ...
    modal: {
      searchBox: {
        resetButtonTitle: "Suchkriterien l\xF6schen",
        resetButtonAriaLabel: "Suchkriterien l\xF6schen",
      ...
      startScreen: {
        ...
        saveRecentSearchButtonTitle: "Zu letzten Suchen hinzuf\xFCgen",
      ...
      errorScreen: {
        ...
        helpText: "M\xF6glicherweise m\xFCssen Sie Ihre Internetverbindung \xFCberpr\xFCfen."
      },
      footer: {
      ...
      noResultsScreen: {
        ...
        suggestedQueryText: "Sie k\xF6nnen versuchen, nach",
...
var frDocsearchLocale = {
  ...
  translations: {
    ...
    modal: {
      searchBox: {
        resetButtonTitle: "Effacer les crit\xE8res de recherche",
        resetButtonAriaLabel: "Effacer les crit\xE8res de recherche",
        cancelButtonText: "Annuler",
        cancelButtonAriaLabel: "Annuler"
      },
      startScreen: {
        recentSearchesTitle: "Recherches r\xE9centes",
        noRecentSearchesText: "Aucune recherche r\xE9cente",
        saveRecentSearchButtonTitle: "Enregistrer dans les recherches r\xE9centes",
        removeRecentSearchButtonTitle: "Supprimer des recherches r\xE9centes",
        favoriteSearchesTitle: "Favoris",
        removeFavoriteSearchButtonTitle: "Supprimer des favoris"
      },
      errorScreen: {
        titleText: "Impossible d'obtenir les r\xE9sultats",
        helpText: "Vous devriez v\xE9rifier votre connexion Internet"
      },
      footer: {
      ...
      noResultsScreen: {
        ...
        reportMissingResultsText: "Pensez-vous que cette recherche devrait avoir des r\xE9sultats ?",
...
import { entries as entries3, fromEntries as fromEntries2, getLocalePaths, getRootLangPath, isPlainObject } from "@vuepress/helper";
function resolveProvideData(app, plugins) {
  ...
  return {
    ...
    __PLUME_PRESET_LOCALE__: fromEntries2(
      entries3(PRESET_LOCALES).filter(([locale]) => locales.includes(locale)).map(([locale, value]) => [locale === root ? "/" : locale, value])
    )
  };
}
...
import { entries as entries4, getRootLangPath as getRootLangPath2, isBoolean, isPlainObject as isPlainObject2 } from "@vuepress/helper";
...
function resolveThemeData(app, options) {
  ...
  options.contributors = isPlainObject2(options.contributors) ? { mode: options.contributors.mode || "inline" } : isBoolean(options.contributors) ? options.contributors : true;
  ...
  if (isPlainObject2(themeData.blog)) {
...
import { ensureLeadingSlash as ensureLeadingSlash3, isPlainObject as isPlainObject3 } from "@vuepress/helper";
var uuid = 1e4;
var cache = {};
...
function autoCategory(page, options) {
  ...
  const blog = isPlainObject3(options.blog) ? options.blog : {};
...
function enableBulletin(page, options) {
  let enablePage;
  if (isPlainObject4(options.bulletin) && options.bulletin.enablePage) {
    enablePage = options.bulletin.enablePage;
  } else if (options.locales) {
    for (const locale of Object.keys(options.locales)) {
      if (isPlainObject4(options.locales[locale].bulletin) && options.locales[locale].bulletin.enablePage) {
...
import { isPlainObject as isPlainObject5 } from "@vuepress/helper";
...
function getPlugins(options, {
  ...
  if (pluginOptions.git ?? isProd) {
    ...
    const changelogOptions = isPlainObject5(options.changelog) ? options.changelog : {};
    plugins.push(gitPlugin({
      createdTime: false,
      updatedTime: options.lastUpdated !== false,
      contributors: isPlainObject5(options.contributors) || options.contributors === true ? {
        avatar: true,
        ...options.contributors === true ? {} : options.contributors
      } : false,
      changelog: options.changelog && (options.docsRepo || changelogOptions.repoUrl) ? { repoUrl: options.docsRepo, ...changelogOptions } : false,
      filter(page) {
  ...
  if (pluginOptions.markdownEnhance !== false) {
    ...
    plugins.push(mdEnhancePlugin(deleteAttrs(options2, "hint", "alert", "imgSize", "imgLazyload", "imgMark", "figure", "obsidianImgSize", "katex", "mathjax", "tabs", "codetabs", "align", "mark", "sub", "sup", "attrs", "tasklist", "footnote")));
  }
  ...
  if (pluginOptions.watermark) {
    plugins.push(watermarkPlugin({
      delay: 300,
      enabled: true,
      ...isPlainObject5(pluginOptions.watermark) ? pluginOptions.watermark : {}
    }));
  }
  ...
  if (pluginOptions.sitemap !== false && isProd) {
    const sitemapOptions = isPlainObject5(pluginOptions.sitemap) ? pluginOptions.sitemap : {};
  ...
  if (pluginOptions.seo !== false && hostname && isProd) {
    const seoOptions = isPlainObject5(pluginOptions.seo) ? pluginOptions.seo : {};
...
import { isPlainObject as isPlainObject6 } from "vuepress/shared";
var PRESET = [
...
async function prepareArticleTagColors(app, localeOptions) {
  const start = performance.now();
  const blog = isPlainObject6(localeOptions.blog) ? localeOptions.blog : {};
...
import { entries as entries5, isLinkAbsolute as isLinkAbsolute2, isLinkHttp, isPlainObject as isPlainObject7 } from "@vuepress/helper";
...
function getSidebarData(app, locales) {
  ...
  entries6(locales).forEach(([localePath, sidebar]) => {
    ...
    } else if (isPlainObject8(sidebar)) {
...
function plumeTheme(options = {}) {
  ...
  return (app) => {
    ...
    return {
      ...
      alias: resolveAlias(),
      plugins: getPlugins(getThemeConfig().localeOptions, { app, pluginOptions, hostname, cache: cache4 }),
